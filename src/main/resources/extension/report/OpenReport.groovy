...
class ORcrosstable {
     ...
     * def avg(records, colName){*     def total=0 as BigDecimal
     *     records.each{rc->
     *         def bd=((rc in Map) ? rc[colName] : rc) as BigDecimal
     *         total=total+bd
     *}*     if(records && records.size()>0){*         return  total.divide(records.size() as BigDecimal, total.scale(), BigDecimal.ROUND_HALF_UP)
     *}*     null
     *}* def max(records, colName){*     def max
     *     records.each{rc->
     *         def bd=((rc in Map) ? rc[colName] : rc) as BigDecimal
     *         max=(max ?: bd).max(bd)
     *}*     max
...
/* see above like 'gender', first set rowspan=1, then will replace this value */
                ...
                summary.eachWithIndex { summaryName, summaryNameValue, summaryIndex -> /* summary the row value, summaryName is the column name to summary the value */
                    println GsonHelper.toJsonString(rowValue)
                    println rowKey
                    def summaryValue = getSummaryBean(summaryName)(rowValue[rowKey], summaryName)
                    /* -FOR TEST- def summaryValue=summaryIndex==0 ? avg(rowValue[columnKey], summaryName) : max(rowValue[columnKey], summaryName) */
                    if (summaryValue != null) {
                        rowsummary[columnIndex * summaryKeys.size() + summaryIndex] << summaryValue
            ...
            colsummary.eachWithIndex { cs, csIdx -> /* append col summary value */
                ...
                if (summaryValue != null) {
                    rowsummary[columnGroupKeys.size() * summaryKeys.size() + csIdx] << summaryValue
                    /* append value to row summary */
                }
...
class ORSQLjdbcConnection {
    def connection(openreportConfig, reportConfig) {
        new groovy.sql.Sql(TCBeanUtil.getSpringBean(reportConfig.datasource))
